...
add_breeze(X,Y,A,B) :- add_location(breeze,X,Y); 
			A is X+1, B is Y, add_location(p_pit,A,Y);
			A is X-1, B is Y, add_location(p_pit,A,Y);
			A is X, B is Y+1, add_location(p_pit,X,B);
			A is X, B is Y-1, add_location(p_pit,X,B).
...
add_visited(X,Y,A,B) :- add_location(visited,X,Y); 
			%locate(visited, X,Y), remove_aroundlocation(X,Y,p_pit,A,B); 
			%locate(visited, X,Y), remove_aroundlocation(X,Y,p_wumpus,A,B); 
			locate(p_wumpus, X,Y), retract(at(p_wumpus, X,Y));
			locate(p_pit, X,Y), retract(at(p_pit, X,Y)).
			
remove_aroundlocation(X,Y,O,A,B) :- 
			A is X+1, B is Y, retract(at(O,A,B));
			A is X-1, B is Y, retract(at(O,A,B));
			A is X, B is Y+1, retract(at(O,A,B));
			A is X, B is Y-1, retract(at(O,A,B)).
